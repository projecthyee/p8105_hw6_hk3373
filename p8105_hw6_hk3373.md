...
For the modeling process, stepwise selection using backward elimination
was performed to determine and select variables to be included in the
proposed linear regression model.

After fitting a regression model with all the variables, backward
elimination removes the least significant predictor for each step, until
the model is left with statistically significant predictors.
...
![](p8105_hw6_hk3373_files/figure-gfm/plot_residuals_fitted_values-1.png)<!-- -->
...
- `model_3` includes head circumference, length, sex, and all
...
![](p8105_hw6_hk3373_files/figure-gfm/plot_cv_prediction_error-1.png)<!-- -->

Model 1 has the lowest RMSE, while model 2 has the highest RMSE out of
the three models. Subsequently, model 1 (the proposed regression model)
is the most appropriate model in predicting child’s birth weight.
